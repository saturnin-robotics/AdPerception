...
Freezes the Sonata-pretrained PTv3 encoder (enc.* + embedding.stem)
and trains the decoder, projection layer, and detection heads.

Usage:
    python scripts/train.py --config local --no-amp
    python scripts/train.py --config jeanzay --wandb
    python scripts/train.py --config jeanzay --resume checkpoints/resume.pth
"""
...
# =========================================================================
# Force spconv to use ConvAlgo.Native on all SubMConv3d layers.
# Fixes implicit_gemm_backward crash on H100 and RTX Ada GPUs.
# Must be applied before any spconv layer is instantiated.
# =========================================================================
import spconv.pytorch as spconv_nn
from spconv.core import ConvAlgo as _ConvAlgo

_original_subm_init = spconv_nn.SubMConv3d.__init__

def _patched_subm_init(self, *args, **kwargs):
    kwargs.setdefault("algo", _ConvAlgo.Native)
    _original_subm_init(self, *args, **kwargs)

spconv_nn.SubMConv3d.__init__ = _patched_subm_init
...
def get_bev_dims(cfg: dict) -> tuple[int, int]:
    """Compute BEV grid H, W from voxel config."""
    pr = cfg["point_range"]
    vs = cfg["voxel_size"]
...
def augment_points_and_boxes(
    ...
    """Standard CenterPoint augmentations: flip Y, rotate Z, scale."""
    pts      = pts.clone()
    ...
    # Random rotation Z in [-pi/4, +pi/4]
...
def build_pipeline(device: torch.device, cfg: dict) -> LiDARPerceptionPipeline:
    """Build pipeline and load Sonata pretrained weights if available."""
...
def freeze_encoder(pipeline: LiDARPerceptionPipeline) -> None:
    """
    Freeze PTv3 encoder layers (enc.* and embedding.stem.*).
    Decoder (dec.*) and projection layer remain trainable.
    """
    n_frozen = 0
    n_total  = 0
    for name, param in pipeline.backbone.backbone.named_parameters():
        n_total += 1
        if name.startswith("enc.") or name.startswith("embedding.stem"):
    ...
    print(f"Total trainable: {n_trainable:,} params")


def trainable_params(pipeline: LiDARPerceptionPipeline) -> list:
    """Collect all parameters with requires_grad=True."""
    decoder_params = [
...
def run_validation(
    ...
    """Single pass over validation set. Skips NaN samples."""
    pipeline.eval()
    val_loss   = 0.0
    n_valid    = 0
    n_nan      = 0
    ...
    if n_nan > 0:
        print(f"  [WARN] {n_nan} NaN/Inf samples skipped during validation")

    return val_loss / max(n_valid, 1)
...
def save_resume_checkpoint(
    path, epoch, pipeline, optimizer, scheduler, scaler,
    best_val_loss, voxel_cfg, args,
) -> None:
    """Save full state for resume after interruption."""
    torch.save({
        "epoch":         epoch,
...
def load_resume_checkpoint(
    path, pipeline, optimizer, scheduler, scaler, device,
) -> tuple[int, float]:
    """Load full state from resume checkpoint."""
...
def main() -> None:
    ...
    parser.add_argument("--no-amp",   action="store_true", default=False,
                        help="disable AMP (for RTX Ada + spconv-cu120)")
    ...
    if use_wandb:
        ...
        wandb.init(
            ...
            id      = f"adperc_{args.config}_{args.version}_lr{args.lr}",
            resume  = "allow",
            config  = {
                "lr": args.lr, "epochs": args.epochs,
                "version": args.version, "config": args.config,
                "amp": use_amp, **voxel_cfg, "bev_H": H, "bev_W": W,
            },
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        print(f"[warn] {args.resume} not found -- starting fresh.")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # === Train ========================================================
        pipeline.train()
        # Keep encoder batchnorm frozen
        pipeline.backbone.backbone.eval()
        # Re-enable decoder train mode
        ...
        for sample in train_loader:
            ...
            # === NaN guard ================================================
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"  [WARN] NaN/Inf at batch {n_batches} -- skipping")
                n_nan += 1
                optimizer.zero_grad(set_to_none=True)
                scheduler.step()
                n_batches   += 1
        ...
        # === Validation ===================================================
        val_loss = run_validation(pipeline, val_dataset, device, voxel_cfg, H, W)
        ...
        if use_wandb:
            wandb.log({
                "epoch/train_loss": avg, "epoch/val_loss": val_loss,
                "epoch/heatmap": hm_avg, "epoch/offset": off_avg,
                "epoch/dims": dims_avg, "epoch/rot": rot_avg,
                "epoch/lr": lr_now, "epoch/nan_skipped": n_nan,
                "epoch": epoch + 1,
            }, step=global_step)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                "epoch": epoch + 1, "state_dict": pipeline.state_dict(),
                "val_loss": best_val_loss, "voxel_cfg": voxel_cfg,
            }, CKPT_OUT)
            saved = f"  --> saved (val={best_val_loss:.4f})"

        # Resume checkpoint
        save_resume_checkpoint(
            CKPT_RESUME, epoch + 1, pipeline, optimizer, scheduler,
            scaler, best_val_loss, voxel_cfg, args,
        )

        nan_info = f" [NaN skipped: {n_nan}]" if n_nan > 0 else ""
        print(
